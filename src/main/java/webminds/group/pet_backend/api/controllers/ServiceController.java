...
public class ServiceController {
    ...
    @GetMapping("pet-shop/{idPet}/{idPetShop}")
    private ResponseEntity<List<ServicePetDto>> getByPetShop(@PathVariable Long idPet ,@PathVariable Long idPetShop){
        Optional<Pet> pet = petService.getById(idPet);

        if(pet.isEmpty()){
            return ResponseEntity.noContent().build();
        }

        List<ServicePet> all = servicePet.getByPetShop(idPetShop, pet.get().getSize());
